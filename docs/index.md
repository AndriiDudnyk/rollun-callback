...
очередей Amazon соответственно. `QueueClient` принимет 3 параметра: 1 - адаптер интерфейса `AdapterInterface`,
2 - название очереди, 3 - количество секунд, которое должно пройти от момента поступления сообщение
в очередь до момента когда сообщение с очереди можна будет извлечь.
Для конфигурации `QueueClient` есть два адаптера: `SqsAdapter` - очереди от Amazon и `FileAdapter` - очерди на файлах.
Обратите внимание на параметр `timeInFlight` (2 параметр в конструкторе `FileAdapter`)
и опцию `VisibilityTimeout` (в массиве 3 параметра `SqsAdapter`). Если эти параметры указаны, то сообщение не будет удалено
и будет доступно снова в очереди по истечению секунд указаных в параметре.
